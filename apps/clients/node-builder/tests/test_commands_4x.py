...
from node_builder.commands._context import prepare_command_context, resolve_deployment
from node_builder.commands.start_tunnel import TUNNEL_TYPES
from node_builder.core.errors import NodeBuilderError
from node_builder.core.eula import SKIP_EULA_ENV
from node_builder.core.workspace import set_working_dir
...
def test_resolve_deployment_requires_region(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("NB_COOKBOOK_PATH", str(tmp_path / "cloud" / "cookbook"))
    monkeypatch.setattr(
        "node_builder.commands._context.validate_environment",
        lambda *a, **k: {
            "AWS_ACCESS_KEY": "a",
            "AWS_SECRET_KEY": "b",
            "TF_VAR_name": "demo",
    ...
    with pytest.raises(NodeBuilderError, match="Region is required"):
