...
from node_builder.terraform.backend import build_backend_config, ensure_backend_resources
from node_builder.terraform.lifecycle import (
...
def test_build_backend_config_s3() -> None:
    ...
    assert "bucket=demo-vs-tfstate-us-east-1" in cfg.args[1]
...
def test_build_backend_config_requires_name() -> None:
    with pytest.raises(NodeBuilderError, match="TF_VAR_name"):
...
def test_ensure_backend_creates_s3_bucket(monkeypatch) -> None:
    ...
    monkeypatch.setattr("node_builder.terraform.backend.run_cmd", fake_run)
    env = {
        "TF_VAR_name": "demo",
        "AWS_ACCESS_KEY": "a",
        "AWS_SECRET_KEY": "b",
...
def test_set_cloud_region_aws(monkeypatch) -> None:
    monkeypatch.setattr(
        "node_builder.terraform.region.validate_region",
        lambda *a, **k: None,
    )
    env: dict[str, str] = {
        "AWS_ACCESS_KEY": "a",
...
def test_terraform_init_passes_backend_args(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "node_builder.terraform.lifecycle.ensure_backend_resources", fake_ensure
    )
    monkeypatch.setattr("node_builder.terraform.lifecycle.run_terraform", fake_run_tf)
