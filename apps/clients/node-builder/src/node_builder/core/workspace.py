...
from node_builder.core.paths import resolve_paths
...
def set_working_dir(
    ...
    *,
    ...
    if not template_dir.exists():
        template_dir.mkdir(parents=True, exist_ok=True)
        # Link recipes when available (in-repo, NB_COOKBOOK_PATH, or image path).
    ...
    *,
    ...
    if external:
        ...
        if not expected_input.is_dir():
            raise NodeBuilderError(f"Invalid input node '{input_node}'.")

        template_dir = (
            ext_root / cookbook_name / "cloud" / "recipes" / recipe_name / cloud
        )
        if not template_dir.is_dir():
            raise NodeBuilderError(f"Invalid recipe template path '{template_dir}'.")
    else:
