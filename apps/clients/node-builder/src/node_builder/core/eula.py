...
def check_eula(
    ...
    *,
    ...
    Set ``NB_SKIP_EULA=1`` to skip (tests / CI).
    """
    ...
    if response in {"q", "Q"} or not response:
        raise NodeBuilderError("EULA was not accepted.")
    if response != "yes":
        raise NodeBuilderError('EULA was not accepted. Type "yes" to accept.')
