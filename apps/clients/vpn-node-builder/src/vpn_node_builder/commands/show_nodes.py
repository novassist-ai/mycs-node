"""``vpnb show-nodes`` — interactive node management."""
...
from vpn_node_builder.cloud.nodes import list_nodes
...
from vpn_node_builder.commands.deploy_node import deploy_node
from vpn_node_builder.commands.destroy_node import destroy_node
...
def show_nodes() -> None:
    ...
    try:
        ...
        if not choice.isdigit() or not (0 <= int(choice) < len(nodes)):
            raise VpnNodeBuilderError("Invalid node selected.")
        ...
        if action not in {"1", "2", "3", "4", "5"}:
            raise VpnNodeBuilderError("Invalid action selected.")
        ...
        if action == "1":
            ...
            if not vpn_ok:
                raise VpnNodeBuilderError("Cannot download VPN config from selected node.")
            ...
            if node.state != "running":
                raise VpnNodeBuilderError("Node is not running.")
            ...
            if node.state == "running":
                ...
                raise VpnNodeBuilderError(f'Cannot change power state from "{node.state}".')
        elif action == "5":
            destroy_node(
                node_type=node.node_type,
                cloud=node.cloud,
